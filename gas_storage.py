...
# Hex versions for HTML table dots
FAC_HEX = {
...
def create_stacked_area_chart(dates, series, bounds, chart_type="stock"):
    ...
    for fac in ordered:
        ...
        # Use stackgroup — Plotly auto-fills from line.color
        # Do NOT set fillcolor explicitly (causes validation errors in some versions)
...
def render_gas_storage_tab():
    ...
    if not any(data.values()):
        st.error("No storage data available. Check API connectivity.")
        return
    ...
    for i, (label, days) in enumerate(preset_options.items()):
        with btn_cols[i]:
            btn_type = "primary" if st.session_state.storage_preset == label else "secondary"
    ...
    for fac in ordered:
        ...
        if b["wv"] > 0:
            fill_pct = str(round((b['cur'] - b['min']) / b['wv'] * 100, 1)) + "%"
        else:
            fill_pct = "n/a"
        rows_html.append(
            '<tr>'
            '<td><span class="storage-dot" style="background:' + hex_col + '"></span>'
